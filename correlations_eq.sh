...
f2fig $map'_'$fw'_smooth_masked_map_eq.fits' -o $map'_'$fw'_smooth_masked_map_eq.gif'
COMMENT
...
do
	...
	difmap -cw $fw_corr $maps_path'src_'$fw'_smooth_masked_map_eq.fits' $maps_path'smica_release3_'$fw'_smooth_masked_map_eq.fits' -o $project_home_path'correlations_results/corr_'$fw_corr'_eq.fits'
	f2fig $project_home_path'correlations_results/corr_'$fw_corr'_eq.fits' -o $project_home_path'correlations_results/corr_'$fw_corr'_eq.gif'
	
	#masking
	mapcut $project_home_path'correlations_results/corr_'$fw_corr'_eq.fits' -rm -90.0d,0d,-5.0d,360.0d -o temp.fits
	mapcut temp.fits -rm 90.0d,0.0d,20.0d -o $project_home_path'correlations_results/corr_'$fw_corr'_masked_eq.fits'
	f2fig $project_home_path'correlations_results/corr_'$fw_corr'_masked_eq.fits' -o $project_home_path'correlations_results/corr_'$fw_corr'_masked_eq.gif'
done
...
do
	...
	#convolving and map generation
	rsalm $map'_alm.fits' -fw $fw -o $map'_'$fw'_smooth_alm.fits'
	#masking
	difalm -eq $map'_'$fw'_smooth_alm.fits' -o $map'_'$fw'_smooth_alm_eq.fits'
	cl2map -falm $map'_'$fw'_smooth_alm_eq.fits' -o $map'_'$fw'_smooth_map_eq.fits' -lmax $lmax -nx $nx -np $np
	mapcut $map'_'$fw'_smooth_map_eq.fits' -rm -90.0d,0d,-5.0d,360.0d -o temp.fits
	mapcut temp.fits -rm 90.0d,0.0d,20.0d -o $map'_'$fw'_smooth_masked_map_eq_'$random'.fits'

	#find correlations between gauss-LCDM map and src map
	for fw_corr in $fwhm_corr
	do
		echo "correlating lcdm "$fw_corr'_'$random
		difmap -cw $fw_corr $maps_path'src_'$fw'_smooth_masked_map.fits' $maps_path'lcdm_map_'$fw'_smooth_masked_map_eq_'$random'.fits' -o $project_home_path'correlations_results/corr_lcdm_'$fw_corr'_'$random'_eq.fits'
		f2fig $project_home_path'correlations_results/corr_lcdm_'$fw_corr'_'$random'_eq.fits' -o $project_home_path'correlations_results/corr_lcdm_'$fw_corr'_'$random'_eq.gif'
	
		#mask-3
		mapcut $project_home_path'correlations_results/corr_lcdm_'$fw_corr'_'$random'_eq.fits' -rm -90.0d,0d,-5.0d,360.0d -o temp.fits
		mapcut temp.fits -rm 90.0d,0.0d,20.0d -o $project_home_path'correlations_results/corr_lcdm_'$fw_corr'_'$random'_masked_eq.fits'
		f2fig $project_home_path'correlations_results/corr_lcdm_'$fw_corr'_'$random'_masked_eq.fits' -o $project_home_path'correlations_results/corr_lcdm_'$fw_corr'_'$random'_masked_eq.gif'
	done
done
